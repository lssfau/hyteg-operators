...
P2PlusBubbleElementwiseMassAnnulusMap::P2PlusBubbleElementwiseMassAnnulusMap( const std::shared_ptr< PrimitiveStorage >& storage,
                                                                              size_t                                     minLevel,
                                                                              size_t maxLevel )
: Operator( storage, minLevel, maxLevel )
{}

void P2PlusBubbleElementwiseMassAnnulusMap::applyScaled( const real_t&                         operatorScaling,
                                                         ...
                                                         UpdateType                            updateType ) const
{
   ...
   else
   {
      for ( auto& it : storage_->getFaces() )
      {
         ...
         applyScaled_P2PlusBubbleElementwiseMassAnnulusMap_macro_2D(
...
void P2PlusBubbleElementwiseMassAnnulusMap::apply( const P2PlusBubbleFunction< real_t >& src,
                                                   const P2PlusBubbleFunction< real_t >& dst,
                                                   uint_t                                level,
                                                   DoFType                               flag,
                                                   UpdateType                            updateType ) const
                                                            ...
                                                            DoFType                                     flag ) const
{
   ...
   else
   {
      ...
      for ( auto& it : storage_->getFaces() )
      {
         ...
         toMatrixScaled_P2PlusBubbleElementwiseMassAnnulusMap_macro_2D(
...
void P2PlusBubbleElementwiseMassAnnulusMap::toMatrix( const std::shared_ptr< SparseMatrixProxy >& mat,
                                                      const P2PlusBubbleFunction< idx_t >&        src,
                                                      const P2PlusBubbleFunction< idx_t >&        dst,
                                                      uint_t                                      level,
                                                      DoFType                                     flag ) const
   ...
   for ( uint_t level = minLevel_; level <= maxLevel_; level++ )
   {
      ...
      else
      {
         ...
         for ( auto& it : storage_->getFaces() )
         {
            ...
            computeInverseDiagonalOperatorValuesScaled_P2PlusBubbleElementwiseMassAnnulusMap_macro_2D(
