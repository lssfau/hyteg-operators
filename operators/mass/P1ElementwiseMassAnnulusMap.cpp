...
P1ElementwiseMassAnnulusMap::P1ElementwiseMassAnnulusMap( const std::shared_ptr< PrimitiveStorage >& storage,
                                                          size_t                                     minLevel,
                                                          size_t                                     maxLevel )
: Operator( storage, minLevel, maxLevel )
{}

void P1ElementwiseMassAnnulusMap::applyScaled( const real_t&               operatorScaling,
                                               ...
                                               UpdateType                  updateType ) const
{
   ...
   else
   {
      for ( auto& it : storage_->getFaces() )
      {
         ...
         applyScaled_P1ElementwiseMassAnnulusMap_macro_2D(
...
void P1ElementwiseMassAnnulusMap::apply( const P1Function< real_t >& src,
                                         const P1Function< real_t >& dst,
                                         uint_t                      level,
                                         DoFType                     flag,
                                         UpdateType                  updateType ) const
                                                  ...
                                                  DoFType                                     flag ) const
{
   ...
   else
   {
      ...
      for ( auto& it : storage_->getFaces() )
      {
         ...
         toMatrixScaled_P1ElementwiseMassAnnulusMap_macro_2D(
...
void P1ElementwiseMassAnnulusMap::toMatrix( const std::shared_ptr< SparseMatrixProxy >& mat,
                                            const P1Function< idx_t >&                  src,
                                            const P1Function< idx_t >&                  dst,
                                            uint_t                                      level,
                                            DoFType                                     flag ) const
   ...
   for ( uint_t level = minLevel_; level <= maxLevel_; level++ )
   {
      ...
      else
      {
         ...
         for ( auto& it : storage_->getFaces() )
         {
            ...
            computeInverseDiagonalOperatorValuesScaled_P1ElementwiseMassAnnulusMap_macro_2D(
