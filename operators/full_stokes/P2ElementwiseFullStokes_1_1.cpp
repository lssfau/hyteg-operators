...
void P2ElementwiseFullStokes_1_1::apply( const P2Function< walberla::float64 >& src,
                                         ...
                                         UpdateType                             updateType ) const
{
   ...
   else
   {
      for ( auto& it : storage_->getFaces() )
      {
         ...
         walberla::float64* _data_muVertex  = face.getData( mu.getVertexDoFFunction().getFaceDataID() )->getPointer( level );
         walberla::float64* _data_muEdge    = face.getData( mu.getEdgeDoFFunction().getFaceDataID() )->getPointer( level );
...
std::shared_ptr< P2Function< walberla::float64 > > P2ElementwiseFullStokes_1_1::getInverseDiagonalValues() const
{
   return invDiag_;
}
